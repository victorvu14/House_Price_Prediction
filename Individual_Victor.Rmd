...
Now we apply fix skewness to all numeric 
```{r}
...
The current MAPE is 0.1258. This is a big improvement from the base model. This could be because the random forest pick up better all the non linearity pattern in the data. Also, dropping of outliers, fix skewness and feature creations show added value here. 
...
traintask <- makeRegrTask(data = xgb_df$train, target = "price")
testTask <- makeRegrTask(data = xgb_df$test, target = "price")
...
dtrain <- xgb.DMatrix(data = as.matrix(xgb_df$train[,-1]),label = final_data$train$price)
dtest <- xgb.DMatrix(data = as.matrix(xgb_df$test[,-1]),label = final_data$test$price)

#Cross validate by 5 folds 
xgbcv <- xgb.cv( params = mytune, data=dtrain, nrounds = 500, nfold = 5, showsd = T, stratified = T, print_every_n = 50, early_stopping_rounds = 20, maximize = F)
...
xgb1 <- xgb.train (params = mytune, data = dtrain, nrounds = xgbcv$best_iteration , watchlist = list(val=dtest,train=dtrain), print.every.n = 50, early.stop.round = 10, maximize = F , eval_metric = "mae")
xgbpred <- as.numeric(exp(predict(xgb1,dtest))-1)

mape(real=as.numeric(exp(xgb_df$test$price)-1), predicted = xgbpred)
```
The MAPE result of the tuned XGBoost model is slightly less than the tuned Random Forrest. Thus we will use the tuned XGBoost to build our final model.
