...
whole_data <- f_partition(df = train_refined,
                        test_proportion = 0.2,
                        seed = 123)
```
...
```{r}
train_refined <- train %>%select(-sqft_living, -sqft_above,-grade, 
                                 -sqft_living15, -sqft_lot, -sqft_basement )
```
...
```{r}
zipcode <- read.csv('zipcode.csv',stringsAsFactors = F)

train_refined<-merge(x=train_refined,y=zipcode,by="zipcode",all.x=TRUE)

for(unique_value in unique(train_refined$City)){
  train_refined[paste("Neighborhood", unique_value, sep = ".")] <- ifelse(train_refined$City == unique_value, 1, 0)
}

#Drop the variable city and zipcode
train_refined <- train_refined %>%select(-zipcode,-City)

str(train_refined)
```

We now have 52 variables in total. 
