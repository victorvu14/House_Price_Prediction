...
ggplot(data.frame(train_refined$bathrooms), aes(x = train_refined$bathrooms))  + 
  geom_density(fill="#e34a33")
...
RF_pred<-predict(Modelrf, newdata = final_data$test, type='response')
rmse(real=final_data$test$price, predicted = RF_pred)
r2(real=final_data$test$price, predicted = RF_pred)
```
